...
const Upload: React.FC = () => {
  ...
  const removeTag = (tagToRemove: string) => {
    handleInputChange('tags', uploadData.tags.filter(tag => tag !== tagToRemove));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        alert(`Model uploaded successfully! Token ID: ${response.tokenId || response.data?.tokenId}`);
        // Reset form
        setUploadData({
          modelName: '',
          description: '',
      ...
      <div className="flex items-center space-x-4">
        {[1, 2, 3, 4].map((stepNum) => (
          <React.Fragment key={stepNum}>
            ...
            {stepNum < 4 && (
              <div className={`w-12 h-1 ${step > stepNum ? 'bg-primary' : 'bg-muted-foreground/50'}`} />
            )}
          </React.Fragment>
        ))}
      </div>
        ...
        <div className="border-2 border-dashed border-border rounded-lg p-6 text-center bg-muted/50">
          ...
          <label htmlFor="model-files" className="cursor-pointer">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Supported formats: GLB, GLTF, OBJ, FBX, STL, DAE, 3DS, BLEND, MAX, MA, MB, C4D, PLY, X3D, SolidWorks (SLDPRT/SLDASM/SLDDRW), AutoCAD (DWG/DXF), STEP/IGES, CATIA, Creo
            </p>
          </label>
        </div>
      ...
      <div>
        ...
        <div className="border-2 border-dashed border-border rounded-lg p-6 text-center bg-muted/50">
          ...
          <label htmlFor="preview-images" className="cursor-pointer">
            <div className="text-muted-foreground mb-2">
              <svg className="mx-auto h-12 w-12" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                <path d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h36v-8a2 2 0 00-2-2H8a2 2 0 00-2 2v8zM6 20l14.293 14.293a1 1 0 001.414 0L42 14M6 20v18a2 2 0 002 2h32a2 2 0 002-2V20" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <p className="text-sm text-muted-foreground">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-muted/50 rounded-lg p-6 space-y-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground capitalize">{uploadData.category}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-muted-foreground">{uploadData.files.length} model file(s)</p>
          <p className="text-muted-foreground">{uploadData.images.length} preview image(s)</p>
        </div>
      </div>

      <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
        <h4 className="font-semibold text-yellow-800 dark:text-yellow-200 mb-2">Before you submit:</h4>
      ...
      <div className="container mx-auto px-4 py-8 pt-24">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-foreground mb-2">Upload Your 3D Model</h1>
